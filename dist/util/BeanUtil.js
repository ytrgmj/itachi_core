...
class BeanUtil {
     ...
     * @param fun
     */
    static shallowCombine(map1, map2, fun) {
        if (map1 == null)
            map1 = {};
        if (map2 == null)
            map2 = {};
    ...
    */
    ...
    static isEq(val1, val2) {
        ...
        if ((val1 instanceof Date || val2 instanceof Date)) {
            val1 = DateUtil_1.DateUtil.parse(val1);
            val2 = DateUtil_1.DateUtil.parse(val2);
            return val1.getTime() == val2.getTime();
        }
        if ((val1 instanceof Array) && (val2 instanceof Array)) {
            return ArrayUtil_1.ArrayUtil.isSame(val1, val2);
        }
    ...
    */
    static copy(src, target, map) {
        ...
        if (map != null && map instanceof Array) {
            map = ArrayUtil_1.ArrayUtil.toMap(map);
        }
        for (let e in src) {
            let val = src[e];
            if (map == null) {
                if (BeanUtil.isPrim(val)) {
                    ...
                    target[e] = lodash_1.default.cloneDeep(val);
                }
            }
            else {
                if (map[e]) {
    ...
    static shallowCloneList(list, map, forbit) {
        let retArray = [];
        if (map != null && map instanceof Array) {
            map = ArrayUtil_1.ArrayUtil.toMap(map);
        }
    ...
    static shallowClone(obj, map, forbit) {
        var ret = {};
        if (obj != null) {
            if (map != null && map instanceof Array) {
                map = ArrayUtil_1.ArrayUtil.toMap(map);
